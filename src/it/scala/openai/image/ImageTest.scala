...
class ImageTest extends FunSuite with TestClient with TestLogger {
  ...
  test("create image edit url response") {
    val image = new File("./resources/test_image.png")
    val n = 3
    ...
    val futureResponse = testClient.image.createEdit(image, request)
  ...
  test("create image variation url response") {
    ...
    val futureResponse = testClient.image.createVariation(image, request)
